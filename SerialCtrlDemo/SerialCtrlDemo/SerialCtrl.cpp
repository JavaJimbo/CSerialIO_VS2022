...
int SerialThread::Run()
{
	...
	while(1)
	{
		...
		while(m_serialIO->GetProcessActivateValue()==TRUE)
		{
			...
			{
				// $$$$ Bug fix: copy selected baud rate to DCB structure:
				m_serialIO->m_DCB.BaudRate = atol((char*)(LPCTSTR)m_serialIO->m_strBaudRate);
...
CSerialIO::CSerialIO():m_serialProcess(NULL),m_strPortName("COM1"),m_strBaudRate("115200"),m_sendSize(0)
{
