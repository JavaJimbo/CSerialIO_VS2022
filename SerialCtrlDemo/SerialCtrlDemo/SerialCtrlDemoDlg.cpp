...
// 
//						Compiled using Visual Studio 2022 (Free version)
//						Adapted from demo source code on Code Project: 
//						"CSerialIO - A Useful and Simple Serial Communication Class" by Tojine
//						Link: https://www.codeproject.com/Articles/99375/CSerialIO-A-Useful-and-Simple-Serial-Communication
// 
//
// 3-25-23 Warwick JBS: $$$$ FIXED BUG: User-selected buadrate wasn't being transferred from combo box.
//						Reading packets from LICOR - works great at 9600 baud.
// 
//						Eliminated List box for displaying read text,
//						Substituted CEdit box to display read text when a carriage return is received.
...
void CSerialCtrlDemoDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT_WRITE, m_editSerialWrite);
	//DDX_Control(pDX, IDC_LIST_READ, m_listboxRead);
	DDX_Control(pDX, IDC_STATIC_INFO, m_staticInfo);
	DDX_Control(pDX, IDC_EDIT_SERIAL_READ, m_editSerialRead);
}
...
{
	int i, val, nLength;
	CString NewString(inPacket);
	static CString strSerialRead = "";  // Buffer for incoming packets


	strSerialRead += NewString;

	
	// If a carriage return is received,
	// then incoming serial packet is complete, 
	// so display it in CEdit box
	// and clear CString buffer to receive next packet:
	if (-1 != strSerialRead.Find("\r"))
	{
		m_editSerialRead.SetWindowTextA(strSerialRead);
		strSerialRead = "";
	}


	CString strInfo;
	strInfo.Format("%d bytes read",inLength);
...
void CSerialCtrlDemoDlg::OnBnClickedButtonWr()
{
	...
	CString strW;
	m_editSerialWrite.GetWindowText(strW);
